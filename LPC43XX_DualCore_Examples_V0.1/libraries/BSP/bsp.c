 ...
 * LED API
 */
/* Make them all weak so that the actual BSP
 * can override them as and when required
 */
void LEDs_TurnOnLEDs(uint32_t LEDMask) __attribute__((weak));
void LEDs_TurnOnLEDs(uint32_t LEDMask) {}
void LEDs_TurnOffLEDs(uint32_t LEDMask) __attribute__((weak));
void LEDs_TurnOffLEDs(uint32_t LEDMask) {}
void _LEDs_ChangeLEDs(uint32_t LEDMask, uint32_t ActiveMask) {}
void LEDs_ChangeLEDs(uint32_t LEDMask, uint32_t ActiveMask) __attribute__((weak, alias("_LEDs_ChangeLEDs")));
void LEDs_ToggleLEDs(uint32_t LEDMask) __attribute__((weak));
void LEDs_ToggleLEDs(uint32_t LEDMask) {}
uint32_t LEDs_GetLEDs(void) __attribute__((weak));
